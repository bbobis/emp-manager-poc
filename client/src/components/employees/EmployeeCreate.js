...
const EmployeeCreate = ({ auth, history }) => {
    ...
    const handleSubmit = event => {
        ...
        console.log('Values: ', firstName, lastName, selectedTitle, selectedDepartments, birthDate);
        ...
        <Container>
            ...
            <Form onSubmit={handleSubmit}>
                ...
                <FormGroup>
                    ...
                    <Input type="select" name="title" value={selectedTitle} onChange={handleChange}>
                        { titles && titles.map(({name, _links}) =>
                            <option key={name} value={_links.self.href}>{name}</option>)
                        }
                    </Input>
                </FormGroup>
                <FormGroup>
                    <Label for="title">Departments</Label>
                    <Input type="select" multiple name="departments" value={selectedDepartments} onChange={handleChange}>
                        { departments && departments.map(({name, _links}) =>
                            <option key={name} value={_links.self.href}>{name}</option>)
                        }
                    </Input>
                </FormGroup>
